...
const Profile = () => {
  ...
  // Upload gambar dan update profil
  const handleImageUpload = async () => {
    ...
    try {
      ...
      const response = await axios.post<{ data: string }>("/api/upload/user", formData, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      });

      const updatedImagePath = response.data.data;

      // Update state user dengan gambar baru
      setUser((prevUser) =>
        prevUser ? { ...prevUser, image: updatedImagePath } : prevUser
      );
          ...
          <div className="relative -mt-20 flex flex-col items-center">
            <div className="relative h-32 w-32 rounded-full border-4 border-white bg-white shadow-lg overflow-hidden">
              ...
              <label
                ...
                className="absolute bottom-0 right-0 flex h-8 w-8 cursor-pointer items-center justify-center rounded-full bg-blue-600 text-white shadow-md hover:bg-blue-700"
              >
                <svg
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
              ...
              <div>
                <div className="mb-4">
                  ...
                  <span className="text-gray-900 dark:text-gray-100">{user.nohp}</span>
                </div>
                <div className="mb-4">
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                    Alamat:
                  </label>
                  <span className="text-gray-900 dark:text-gray-100">{user.alamat}</span>
                </div>
                <div className="mb-4">
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300">
                    Level:
                  </label>
                  <span className="text-gray-900 dark:text-gray-100">{user.level}</span>
                </div>
              </div>
